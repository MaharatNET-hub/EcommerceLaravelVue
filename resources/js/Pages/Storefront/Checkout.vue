...
<template>
    <Seo title="إتمام الطلب" robots="noindex,follow" />

    <div class="mx-auto max-w-5xl px-4 py-10">
        <h1 class="mb-6 text-2xl font-extrabold text-gray-800">إتمام الطلب</h1>
            ...
            <form @submit.prevent="submit" class="space-y-4 lg:col-span-2">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-sm font-medium text-gray-700">العنوان التفصيلي</label>
                        <input v-model="form.address_line" type="text" class="w-full rounded-lg border-gray-300" required />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">الرمز البريدي (اختياري)</label>
                        <input v-model="form.postal_code" type="text" class="w-full rounded-lg border-gray-300" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700">كوبون الخصم (اختياري)</label>
                        <input v-model="form.coupon_code" type="text" class="w-full rounded-lg border-gray-300" />
                    </div>
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-sm font-medium text-gray-700">ملاحظات (اختياري)</label>
                ...
                <div class="rounded-lg border border-gray-100 p-4 text-sm text-gray-600">
                    الدفع عند الاستلام (COD) هو طريقة الدفع الحالية المتاحة.
                </div>
